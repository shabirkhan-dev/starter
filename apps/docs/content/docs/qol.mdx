...
description: Hooks, CI, and developer quality tooling across the monorepo.
---
...
|----------|----------------|--------------|
| **TS/JS** | oxlint (lint), oxfmt (format) | tsc (typecheck), Vitest/Jest if added |
...
- **`bun run format`** — oxfmt + scripts (shfmt, ruff) + cargo fmt
